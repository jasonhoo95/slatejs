		...
		var webView = {
			...
			prepare: function (settings) {
				...
				document.addEventListener("fullscreenchange", function (event) {
					...
					if (
						...
						document.fullscreenElement.id == iframeId
					) {
						webView.isFullscreen = true;
						window.flutter_inappwebview.nativeCommunication(
							"onEnterFullscreen",
							viewId
						);
					...
					iframe.addEventListener("load", function (event) {
						...
						try {
							...
							iframe.contentWindow.open = function (
								...
								var newWindow = originalOpen.call(
								...
								window.flutter_inappwebview
									.nativeCommunication("onCreateWindow", viewId, [
										windowId,
										url,
										target,
										windowFeatures,
						...
						try {
							...
							if (
								!webView.settings.verticalScrollBarEnabled &&
								!webView.settings.horizontalScrollBarEnabled
							) {
							...
							if (webView.settings.disableContextMenu) {
								iframe.contentWindow.addEventListener(
									"contextmenu",
									webView.disableContextMenuHandler
								);
							}
						} catch (e) {
							console.log(e);
			...
			setSettings: function (newSettings) {
				...
				try {
					...
					if (
						webView.settings.verticalScrollBarEnabled !=
							newSettings.verticalScrollBarEnabled &&
						webView.settings.horizontalScrollBarEnabled !=
							newSettings.horizontalScrollBarEnabled
					) {
						if (
							!newSettings.verticalScrollBarEnabled &&
							!newSettings.horizontalScrollBarEnabled
							...
							var styleElement = iframe.contentDocument.getElementById(
								"settings.verticalScrollBarEnabled-settings.horizontalScrollBarEnabled"
							);
							if (styleElement) {
								styleElement.remove();
					...
					if (
						webView.settings.disableVerticalScroll !=
					...
					if (
						webView.settings.disableHorizontalScroll !=
					...
					if (
						webView.settings.disableContextMenu !=
						newSettings.disableContextMenu
					) {
			...
			getSize: function () {
				...
				if (
					...
					iframeContainer.style.width.indexOf("px") > 0
				) {
				...
				if (
					...
					iframeContainer.style.height.indexOf("px") > 0
				) {
