...
export default function KatexPop({ type, quill, dataValue, open, setOpen }) {
  ...
  const renderLinkInput = useMemo(() => {
    return (
      <div>
        ...
        <input type='text' value={linkInput} tabIndex={-1} onChange={(e) => setLink(e.target.value)} />
  ...
  const renderKatexInput = useMemo(() => {
    return (
      <div>
        <h1 className='helloWorld'>KATEX VAL: {dataValue.value ? dataValue.value : null}</h1>
        <span>Katex Input:</span>
        <input type='text' tabIndex={-1} value={katexValue} onChange={(e) => setKatex(e.target.value)} />{' '}
        <button style={{ outline: 'none' }} onClick={(e) => katexInsert()}>
          INSERT Katex Text
        </button>
      </div>
    );
