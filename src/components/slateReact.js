        ...
        if (
            ...
            Transforms.mergeNodes(editor, {
                ...
                match: (n) =>
                    ...
                    ['banner-red-wrapper'].includes(n.type),
            });
            const [listItems] = Editor.nodes(editor, {
                at: editor.selection.anchor.path,
            ...
            previousParent &&
            previousParent[0].type == 'check-list-item' &&
            ...
            const [cell] = Editor.nodes(editor, {
                match: (n) =>
                    !Editor.isEditor(n) && SlateElement.isElement(n) && n.type === 'table-cell1',
            });
                ...
                if (edges[0][0] === edges[1][0] && edges[0][2] === edges[1][2]) {
                    ...
                    for (const [parent, path] of Editor.nodes(editor, {
                        ...
                        for (const [value, childPath] of Node.children(editor, path, {
                            ...
                            console.log(childPath, 'remove child path node');
                ...
                onChange={(value) => {
                    ...
                    if (editor.selection) {
                        ...
                        if (parent[0].type == 'link' && parent[0].children[0].text.length <= 0) {
                            ...
                            (block || endBlock) &&
                            block &&
                            editor.selection.anchor.path[0] === block[1][0] &&
                            ...
                            if (focusTable) {
                                for (const [parent, path] of Editor.nodes(editor, {
                                    ...
                                    if (
                                        editor.selection.anchor.path[0] <
                                            editor.selection.focus.path[0] &&
                                        path[1] === focusTable[0].children.length - 1
                                    ) {
                                        value.push({ offset: parent.text.length, path: path });
                                    } else if (
                                        editor.selection.anchor.path[0] >
                            ...
                            for (const [parent, path] of Editor.nodes(editor, {
                                ...
                                if (
                                    editor.selection.anchor.path[0] <
                                        editor.selection.focus.path[0] &&
                                    endBlock[0].children.length - 1 === path[1]
                                ) {
                ...
                <Editable
                    ...
                    onKeyDown={(event) => {
                        ...
                        if (event.key == 'Enter' && event.shiftKey && parentCheck) {
                            ...
                            event.key == 'Enter' &&
                            listItems &&
                            ['editable-void', 'ImageWrapper'].includes(listItems[0].type)
                        ) {
                            event.preventDefault();
                            Transforms.setNodes(
                                editor,
                                { checked: false, selectNode: true },
                                { at: listItems[1] },
                            );
...
const withInlines = (editor) => {
    ...
    editor.isInline = (element) =>
        ['button', 'link', 'katex', 'inline-bug', 'inline-wrapper-bug', 'inline-wrapper'].includes(
            element.type,
        ) || isInline(element);
...
const toggleBlock = (editor, format, type) => {
    ...
    if (
        ...
        Transforms.mergeNodes(editor, {
            at: parentPath,
            match: (n) =>
                !Editor.isEditor(n) && SlateElement.isElement(n) && n.type == currentNode[0].type,
        });
    ...
    function checknow(event) {
        if (event && typeof event.data == 'katexnow') {
            ...
            if (value && value.id == 'katex') {
                ...
                Transforms.setNodes(
                    editor,
                    { card: [{ card: value.card, id: value.cardId, check: false }] },
                    { at: path },
                );
...
const ImageWrapper = ({ attributes, children, element }) => {
    ...
    return (
        <div
            style={{ border: selected ? '3px solid blue' : '' }}
            className='h-[100px] w-[100px] relative overflow-hidden'
            {...attributes}>
            <div
                className='w-full h-full absolute left-0 top-0 z-[2] overflow-hidden'
                contentEditable='false'>
                <img
