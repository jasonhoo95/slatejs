...
const SlateReact = () => {
            ...
            if (anchor && focus) {
                ...
                if (
                    ...
                    (selection.anchor.path[1] != selection.focus.path[1] ||
                        selection.anchor.path[2] != selection.focus.path[2])
                ) {
    ...
    editor.deleteBackward = (...args) => {
        ...
        const listItemCheck = Editor.above(editor, {
            match: (n) =>
                n.type == 'list-item' || n.type == 'paragraph' || n.type === 'ImageWrapper' || n.type === 'table-list',
        });
        ...
        if (
            ...
            Transforms.mergeNodes(editor, {
                at: listItemParent[1],
                match: (n) =>
                    !Editor.isEditor(n) && SlateElement.isElement(n) && ['banner-red-wrapper'].includes(n.type),
            });
            const [listItems] = Editor.nodes(editor, {
                at: editor.selection.anchor.path,
            ...
            <Slate
                ...
                <div
                    onClick={(e) => {
                        ...
                        const [listItems] = Editor.nodes(editor, {
                            match: (n) =>
                                n.type === 'paragraph' || n.type == 'list-item' || n.type == 'banner-red-wrapper',
                        });
                ...
                <div
                    onClick={(e) => {
                        ...
                        Transforms.unwrapNodes(editor, {
                            split: true,
                            match: (n) => n.type === 'numbered-list' || n.type === 'check-list',
                        });
                        Transforms.insertNodes(editor, block);
                ...
                <Editable
                    ...
                    onKeyDown={(event) => {
                        ...
                        const parentCheck = Editor.above(editor, {
                            match: (n) =>
                                n.type == 'list-item' || n.type == 'paragraph' || n.type === 'check-list-item',
                        });
...
const KatexComponent = ({ attributes, children, element }) => {
    ...
    return (
        <span
            ...
            className='span-katex select-none'
            {...attributes}>
            <span
    ...
    if (format == 'numbered-list') {
        return (
            <div
                style={{
                    padding: '10px',
                    color: isBlockActive(editor, format, TEXT_ALIGN_TYPES.includes(format) ? 'align' : 'type')
                        ? 'red'
                        : 'black',
                }}
                onMouseDown={(event) => {
...
const TableList = ({ attributes, children, element }) => {
    ...
    const [endPath] = Editor.nodes(editor, {
        match: (n) => n.type === 'table-list',
    ...
    return (
        <div
            className={`inline-flex relative my-2 mx-2 ${(!startPath || !endPath) && selected ? 'table-wrapper' : ''}`}>
            <table
                className={`table-list w-full relative ${(!startPath || !endPath) && selected ? 'bg-sky-200' : ''}`}
                {...attributes}>
                <tbody>
