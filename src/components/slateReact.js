...
const SlateReact = () => {
	...
	editor.insertBreak = () => {
		...
		// }
			...
			currentParent &&
			['check-list-item'].includes(currentParent[0].type)
		) {
			insertBreak();
			const parentCheck = Editor.parent(editor, editor.selection.anchor.path, {
			...
			currentParent &&
			['editable-void', 'ImageWrapper'].includes(currentParent[0].type)
		) {
			Transforms.setNodes(
				editor,
		...
		if (
			...
			if (listItems) {
				...
				nextnode = Editor.next(editor, {
					at: listItems[1],
					match: (n) =>
						['paragraph', 'numbered-list', 'bulleted-list'].includes(n.type),
				});
			...
			(listItemParent[0].type == 'list-item' ||
				listItemParent[0].type == 'check-list-item') &&
			!previousKatex &&
	...
	editor.deleteFragment = (...args) => {
		...
		if (
			checkListItem &&
			!['list-item', 'check-list-item'].includes(checkListItem[0].type)
		) {
			Transforms.setNodes(
				editor,
				{ type: 'paragraph' },
				...
				onChange={(value) => {
					...
					if (editor.selection) {
						...
						if (
							...
							parent[0].type == 'link' &&
							parent[0].children[0].text.length <= 0
						) {
				...
				<Editable
					...
					onKeyDown={(event) => {
						...
						const parentCheck = Editor.above(editor, {
							match: (n) =>
								n.type == 'table-cell1' ||
								n.type == 'dropdown-inner' ||
								n.type == 'numbered-list',
						});
						const stringText = Editor.node(
							editor,
							editor.selection.anchor.path,
						);
						...
						if (
							...
							event.shiftKey &&
							listItems &&
							(listItems[0].type == 'list-item' ||
								listItems[0].type == 'check-list-item')
						) {
							event.preventDefault();
							const nextNode = Editor.next(editor, {
								at: editor.selection.anchor.path,
							...
							listItems &&
							(listItems[0].type == 'editable-void' ||
								listItems[0].type == 'ImageWrapper') &&
							event.key == 'Enter'
						) {
							event.preventDefault();
							Transforms.move(editor, { distance: 1, unit: 'offset' });
						...
						// }
							...
							stringText[0].text.match(pattern) &&
							/android/i.test(ua) &&
							!parentCheck
						) {
							setTimeout(() => {
...
const EditableVoid = ({ attributes, children, element }) => {
	...
	return (
		...
		>
			...
			<div
				className='h-full w-full absolute z-[10] left-0 top-0'
				contentEditable='false'
			>
				<div
					onClick={(e) => {
